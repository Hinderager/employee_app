...
export default function HazardousDropPage() {
  ...
  // Fetch hazardous collection data
  useEffect(() => {
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !data?.hasCollectionToday || loading) return;
  ...
  const initMap = () => {
    if (mapRef.current || !data?.locations.length) return;
        ...
        <>
          ...
          <div className="bg-amber-50 border-b border-amber-200 px-4 py-3">
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-amber-800 font-medium">Collection: {data.hours}</span>
            </div>
          </div>
